Public Class frmCstmHV
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If chk65kASCCRBase.Checked Then
            ModuleCodeList.Add("920F6A")
        End If

        If chk30kASCCRBase.Checked Then
            ModuleCodeList.Add("920F6B")
        End If

        Call PerformDesignCautionScan(False)
        For i = 0 To ModuleCodeList.Count - 1
            frmMain.ThisUnitCstmHVCodes.Add(ModuleCodeList.Item(i))
    ...
    Private Sub WriteHistory()
        ...
        For i = 1 To lstItemsInDB.SelectedItems.Count
            ...
            MySQL = "Select * FROM tblHistoryCstmPower WHERE (JobName='" & jname & "') AND (UnitID='" & unit & "') AND (Version='" & ver & "') AND (CustomCode='" & CstmCode & "')"
            rs.Open(MySQL, con)

            If Not (rs.EOF And rs.BOF) Then
                'Update SQL
                ExistingRecordID = rs.Fields(0).Value
                MySQL = "UPDATE tblHistoryCstmPower SET Controller='" & Controller & "', CustomCode='" & CstmCode & "', " & "CustomDescription='" & CstmDesc & "' WHERE ID=" & ExistingRecordID
                con.Execute(MySQL)
            Else
                'Insert SQL
                MySQL = "INSERT INTO tblHistoryCstmPower (JobName,UnitID,Version,ModelNumber,CustomCode,CustomDescription,Controller) VALUES ('" & jname & "','" & unit & "','" & ver & "','" & modnum & "','" & CstmCode & "','" & CstmDesc & "','" & Controller & "')"
                con.Execute(MySQL)
            End If
            rs.Close()
    ...
    Private Sub PerformDesignCautionScan(Prelim As Boolean)
        ...
        For i = 0 To ModuleCodeList.Count - 1
            ...
            If Prelim Then
                MySQL = "SELECT COUNT(*) as RowCount FROM tblDesignCautions WHERE TriggerCode LIKE '920%'"
            Else
            ...
            If RecCount > 0 Then
                If Prelim Then
                    MySQL = "SELECT * FROM tblDesignCautions WHERE TriggerCode LIKE '920%'"
                Else
    ...
    Private Sub cmdScience_Click(sender As Object, e As EventArgs) Handles cmdScience.Click
        Dim dummy As MsgBoxResult
        dummy = MsgBox("Science! for the Custom Power Module is not yet implemented.")
        WriteToEventLog("CstmHV", "Science! for the Custom Power Module is not yet implemented.", "Development", Str(dummy))
    End Sub
    ...
    Private Sub FIOPPreview()
        ...
        'Update next line
        MySQL = "SELECT * FROM tblFisenInstalledOptions WHERE FIOpCode Like '920%'"
        rs.Open(MySQL, con)

        'Update next line
        TempModList.Add("920000")
