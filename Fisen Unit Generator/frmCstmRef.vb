Public Class frmCstmRef
    ...
    Private Sub PerformDesignCautionScan()
        ...
        For i = 0 To lstItemsInDB.SelectedItems.Count - 1
            MySQL = "SELECT COUNT(*) as RowCount FROM tblCstmRef INNER JOIN tblDesignCautions ON tblCstmRef.CstmCode = tblDesignCautions.TriggerCode WHERE tblCstmRef.CstmFIOP='" & lstItemsInDB.SelectedItems.Item(i) & "'"
            rs.Open(MySQL, con)
            Reccount = rs.Fields("RowCount").Value
            rs.Close()

            If Reccount > 0 Then
                MySQL = "SELECT tblCstmRef.CstmCode as CstmCode, tblCstmRef.CstmFIOP as CstmFIOP, tblDesignCautions.ShortName as ShortName, tblDesignCautions.LongText as MyLongText, tblDesignCautions.TriggerCode as TriggerCode FROM tblCstmRef INNER JOIN tblDesignCautions ON tblCstmRef.CstmCode = tblDesignCautions.TriggerCode WHERE tblCstmRef.CstmFIOP='" & lstItemsInDB.SelectedItems.Item(i) & "'"
                rs.Open(MySQL, con)
    ...
    Private Sub UpdateCodeList(locCode As String)
        ...
        frmMain.ThisUnitCstmRefCodes.Add(locCode)
        For i = 0 To pTagALongParent.Count - 1
            If pTagALongParent.Item(i) = locCode Then
                frmMain.ThisUnitCstmRefCodes.Add(pTagALongsSelected.Item(i))
            End If
        Next
    End Sub
